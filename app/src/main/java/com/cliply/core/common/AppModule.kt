...
import com.cliply.download.resolver.*
...
object AppModule {
    @Provides @Singleton fun database(@ApplicationContext context: Context): CliplyDatabase = Room.databaseBuilder(context, CliplyDatabase::class.java, "cliply.db").addMigrations(MIGRATION_1_2, MIGRATION_2_3, MIGRATION_3_4).fallbackToDestructiveMigration().build()
    ...
    @Provides @Singleton fun mediaResolverRegistry(client: OkHttpClient): MediaResolverRegistry = MediaResolverRegistry(listOf(ControlledTestMediaResolver(com.cliply.BuildConfig.TEST_MEDIA_URL), DirectHttpsMediaResolver(client)))
    @Provides @Singleton fun mediaSourceResolverRegistry(client: OkHttpClient): MediaSourceResolverRegistry = MediaSourceResolverRegistry(listOf(DirectMediaSourceResolver(DirectHttpsMediaResolver(client)), AuthorizedInstagramSourceResolver(UnconfiguredAuthorizedInstagramResolver())))
